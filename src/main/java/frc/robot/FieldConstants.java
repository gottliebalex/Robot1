...
public class FieldConstants {
  ...
  public static final class Reef {
    ...
    /**
     * Returns the algae pose for a given branch using the desired mode. Automatically selects the
     * algae level (L2/L3) and side based on the configured per-face placement maps
     * (GrabALGAE_PLACEMENT / SCALGAE_PLACEMENT). The provided {@code level} is only used as a
     * fallback if a placement is missing for the branch.
     */
    public static Pose2d algaePose(Branch branch, Level level, AlgaeMode algaemode) {
      ...
      if (placement != null) {
        return CURRENT_SCORING_SIDE.get(branch).get(placement.level()).get(placement.side());
      }
    ...
    public static Pose2d algaePose(Branch branch, int level, AlgaeStandoff standoff) {
      ...
      AlgaeMode mode =
          (standoff == AlgaeStandoff.SUPERCYCLE) ? AlgaeMode.SUPERCYCLE : AlgaeMode.GRAB;
